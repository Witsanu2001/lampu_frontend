  ...
  const handleConfirmPayment = async () => {
    ...
    if (isAdmin) {
      ...
      if (adminAddressDetail.trim() === "") {
        const confirmProceed = window.confirm(
          "ยังไม่ได้กรอกรายละเอียดที่อยู่ แน่ใจหรือไม่ที่จะไปต่อ?",
        );
    ...
    const orderData = {
      ...
      shipping: {
        ...
        address: isAdmin
          ? isMeetup
            ? `[นัดรับนอกสถานที่] ${adminAddressDetail}`
            : adminAddressDetail
        ...
        <div className="fixed inset-0 z-50 bg-white">
          <SelectMaps
            onLocationConfirm={(lat, lng, fee, dist, meetup) => {
              ...
              setIsMeetup(meetup);
              setShowMap(false);
              setApplyDeliveryFee(true); // 🌟 บังคับติ๊กถูกให้อัตโนมัติเผื่อแอดมินลืมติ๊ก
            }}
          />
          <button 
                  ...
                  <div className="mt-4 pt-4 border-t border-gray-100 dark:border-gray-700">
                    ...
                    <label className="flex items-center gap-3 mb-3 cursor-pointer w-max">
                      ...
                      <span className="font-bold text-gray-800 dark:text-white">
                        คิดค่าจัดส่ง (ถ้าไม่ติ๊ก = ส่งฟรี 0 บาท)
                      </span>
                    </label>

                    {/* แสดงส่วนของแผนที่ตลอดเวลา เหมือนแบบเดิม */}
                        ...
                        {location ? (
                          <div>
                            <p className="text-green-600 dark:text-green-400 font-semibold flex items-center gap-2">
                              <span>✅ ปักหมุดระยะทางเรียบร้อย</span>
                            </p>
                            ...
                            {/* แจ้งเตือนถ้าปักหมุดแล้วแต่ลืมติ๊กคิดค่าส่ง */}
